...
public class StudentController {
	...
	public ResponseEntity<?> deleteStudent(@RequestParam int id){
		try{
			...
			ErrorClazz errorClazz=new ErrorClazz(4, "Unable to delete student details for the id "+id);
			//errorclazz object will get converted into JSON and added in the body of HttpResponse
			return new ResponseEntity<ErrorClazz>(errorClazz,HttpStatus.INTERNAL_SERVER_ERROR);
			//response.data={errorCode:4,message:"Unable to delete student details for the id"+id}
			//response.status=500
		}
	}
	...
	public ResponseEntity<?> updateStudent(@RequestBody Student student){
		try{
			...
			ErrorClazz errorClazz=new ErrorClazz(5,"Unable to update student details "+e.getMessage());
			return new ResponseEntity<ErrorClazz>(errorClazz,HttpStatus.INTERNAL_SERVER_ERROR);
